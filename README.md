...
6. Craft a **catalog fragment** (paper, amethyst shard, and ink sac, stacked) and use it to open the tags and patterns you have already learned. An observatory chest can also hold one. The screen shows only what that player has already discovered. Craft a **field guide** (a book over an amethyst shard) and use it for illustrated pages on the same loop: imprints, bands, crafts, formulas, the three mobs, worldgen, and the short multiplayer rules. The pictures are drawn by the client. The dedicated server only registers the item. An observatory chest can hold the book as well.
