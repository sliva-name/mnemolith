...
A fractured chunk remembers too much at once. Once a second, for each player standing in one, `stormAttemptChance` (0.02) may start a **recollection storm** there; freeing a residual shard on a fractured chunk calls one at once. A storm covers the 3×3 chunks around its centre and shows a boss bar to everyone within 48 blocks. It **gathers** for 10 seconds: a mute stone in the centre chunk, or scar glass within one chunk, contains it before it breaks. Then it **rages** for six waves, one every 10 seconds: each wave condenses the area's loudest body memories into storm residues (up to 6), and each one acts its memory out (a zombie, a fire, a small blast, a lift, darkness; fire and blasts respect `mobGriefing`) unless it is starved in a muted chunk, drunk by a grafted echo of its temper, read and pinned, or swallowed by a hushed echo. Draining the area first, reading and capturing storm residues (each capture is a shard), and a muted centre (every wave there counts twice) all make it end **spent** or **passed**. If three or more storm residues outlast the waves they merge into **the Scar**.
...
./gradlew runClient
./gradlew runServer
./gradlew runGameTestServer   # all QA suites and live residue and storm tests, headless; non-zero exit on failure
```
...
**Бури воспоминаний и Шрам.** Разлом помнит слишком много сразу. Раз в секунду на каждого игрока в разломе `stormAttemptChance` (0,02) может начать там **бурю воспоминаний**; выпущенный в разломе осколок осадка вызывает её сразу. Буря накрывает 3×3 чанка вокруг центра, её полосу видят все в 48 блоках. 10 секунд она **собирается**: глушащий камень в центральном чанке или шрамовое стекло в пределах чанка гасят её до начала. Потом шесть **волн** раз в 10 секунд: каждая сгущает самые громкие памяти о теле в осадки бури (до 6), и каждый разыгрывает свою память (зомби, огонь, малый взрыв, подъём, тьма; огонь и взрывы подчиняются `mobGriefing`), если его не заморил заглушённый чанк, не выпил привитый отголосок того же нрава, не сковало чтение и не проглотил безмолвный отголосок. Осушите место заранее, читайте и ловите осадки бури (каждый — осколок), заглушите центр (там каждая волна считается дважды) — и буря иссякнет или пройдёт. Если после волн устояли три осадка или больше, они сливаются в **Шрам**.
...
Одинаковый jar нужен и клиенту, и выделенному серверу. Серверный конфиг задаёт правила бурь (`allowRecollectionStorms`, `maxStormsPerDimension`); шанс бури `stormAttemptChance` и порог `recollectionStormThreshold` лежат в общем конфиге, `imprintNodeWeight` пока зарезервирован. Клиентский конфиг (частицы, плашка линзы, виньетка, тряска, бахрома разлома, громкость) остаётся на компьютере игрока. Каталог открывается пакетом с сервера. Классы из `com.mnemolith.client` на выделенном сервере не загружаются.
