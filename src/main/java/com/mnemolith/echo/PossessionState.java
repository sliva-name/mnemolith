...
public final class PossessionState {
    ...
    public record Body(UUID echo, float maxHealth, Optional<EchoRecording> recording, Optional<com.mnemolith.echo.job.EchoJob.Saved> job,
            Optional<com.mnemolith.echo.graft.EchoGraft> graft, boolean scarred, Optional<UUID> relay) {
        static final Codec<Body> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                ...
                Codec.BOOL.optionalFieldOf("scarred", false).forGetter(Body::scarred),
                UUIDUtil.CODEC.optionalFieldOf("relay").forGetter(Body::relay))
                .apply(instance, Body::new));

        public Body(UUID echo, float maxHealth, Optional<EchoRecording> recording) {
            this(echo, maxHealth, recording, Optional.empty(), Optional.empty(), false, Optional.empty());
        }
        ...
        public Body withGraft(Optional<com.mnemolith.echo.graft.EchoGraft> value) {
            return new Body(this.echo, this.maxHealth, this.recording, this.job, value, this.scarred, this.relay);
        }
    }
