...
/** Client-side shell: carries the avatar state and the owner's skin for {@link EchoRenderer} (drawn opaque, slightly greyed). */
public class ClientEchoShell extends EchoShell implements ClientAvatarEntity {
    private final ClientAvatarState avatarState = new ClientAvatarState();
    private final AvatarSkin skin = new AvatarSkin();

    public ClientEchoShell(EntityType<? extends EchoShell> type, Level level) {
        super(type, level);
    }
