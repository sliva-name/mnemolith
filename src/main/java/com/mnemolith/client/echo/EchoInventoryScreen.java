 ...
 * (Повтор / Добыча / Стройка / Стоп), mining radius, the linked chest, the blueprint, and the live status.
...
public class EchoInventoryScreen extends AbstractContainerScreen<EchoMenu> {
    ...
    public EchoInventoryScreen(EchoMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title, PANEL_X + PANEL_W, EchoMenu.HEIGHT);
    ...
    protected void init() {
        ...
        this.replay = this.addRenderableWidget(Button.builder(Component.translatable("mnemolith.gui.echo.mode.replay"), b -> this.send(EchoJobPayload.Action.MODE_REPLAY))
                .bounds(x, y + 41, BUTTON_W, 16).tooltip(Tooltip.create(Component.translatable("mnemolith.gui.echo.mode.replay.tip"))).build());
        ...
        this.unlink = this.addRenderableWidget(Button.builder(Component.translatable("mnemolith.gui.echo.unlink"), b -> this.send(EchoJobPayload.Action.UNLINK_CHEST))
                .bounds(right, y + 111, BUTTON_W, 16).build());
        this.place = this.addRenderableWidget(Button.builder(Component.translatable("mnemolith.gui.echo.place"), b -> this.onPlace())
                .bounds(x, y + 142, BUTTON_W * 2 + 4, 16).tooltip(Tooltip.create(Component.translatable("mnemolith.gui.echo.place.tip"))).build());
        this.refreshButtons();
    }
    ...
    private void extractJobPanel(GuiGraphicsExtractor graphics) {
        ...
        List<Component> lesson = this.lesson().describeShort();
        for (int i = 0; i < Math.min(2, lesson.size()); i++) {
        ...
        GuiArt.label(graphics, this.font, Component.translatable("mnemolith.gui.echo.radius", echo.jobRadius()), x, 84, GuiArt.BONE);
        Component chest = echo.jobChest().map(EchoInventoryScreen::pos).orElse(Component.translatable("mnemolith.gui.echo.none"));
        this.fitted(graphics, Component.translatable("mnemolith.gui.echo.chest").append(": ").append(chest), x, 100, width,
        ...
        this.fitted(graphics, Component.translatable("mnemolith.gui.echo.blueprint").append(": ").append(anchor), x, 131, width,
                echo.jobAnchor().isPresent() ? GuiArt.BONE : DIM);
        GuiArt.label(graphics, this.font, Component.translatable("mnemolith.gui.echo.status"), x, 163, GuiArt.BONE);
        List<FormattedCharSequence> lines = this.font.split(echo.jobStatus(), width);
        int color = echo.jobStopped() ? WARM : PINK;
        for (int i = 0; i < Math.min(2, lines.size()); i++) {
            graphics.text(this.font, lines.get(i), x + 1, 175 + i * 10, GuiArt.SHADOW, false);
            graphics.text(this.font, lines.get(i), x, 174 + i * 10, color, false);
        }
    }
