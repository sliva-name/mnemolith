...
Phase 8 art for the objects the player actually holds, places, and fights. Geometry stays Minecraft-shaped. Hierarchy and palette do the work. There is no GeckoLib dependency and no `AnimationDefinition` clip. Mob motion is `EntityModel.setupAnim` posing the parts already synced through `MobActions`.
