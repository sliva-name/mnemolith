...
- The lens epoch moves only when cached pressure or the fractured flag actually changes. A memory change always marks the chunk unsaved, even when the clamped score stays put. Loading a chunk that recomputes to the same score, with nothing faded, does not dirty the attachment.
...
- A lens request that still matches the player's memory epoch, chunk, and the server's lens or ambient decision does not walk chunks and does not resend the snapshot. Ambient without a lens is `gameplay.allowAmbientPressure`, not the client's request bit. When only the epoch moved, the same player, chunk, and mode are walked at most once per 10 ticks (`PressureSync.MIN_REWALK_TICKS`), whatever the client's poll rate; a new chunk or a lens change is answered at once. Vein particles still repeat every `gameplay.veinShimmerTicks` (default 40) while the lens is held. 0 repeats only when the snapshot is new. A changed chunk still shimmers on the next poll.
