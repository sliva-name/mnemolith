...
| `com.mnemolith.entity.echo` | both | Echo and shell entities (`MemoryAvatar`, `EchoEntity`, `EchoShell`), the echo's 41-slot inventory, the residual echo (`ResidueEntity`: state, timers, drift; rules in `echo.residue`), and the Scar boss (`ScarEntity`: reading, pin, recall, leash; storm rules in `echo.storm`) |
...
Fracture sets `ChunkMemory.fractured`, logs `Mnemolith fracture`, and asks `MobSpawns.trySpawnReplicant` once for that chunk if no replicant is already within 24 blocks. `server.logPressureChanges` logs other band changes. A fractured chunk is the only place a recollection storm can gather (see [Recollection storm](#recollection-storm)); `difficulty.recollectionStormThreshold` scales the bands, so it also decides how loud a chunk must get first. `spawnRates.imprintNodeWeight` is still reserved: no worldgen feature places imprint nodes, and its comment says so.
...
Every rule is in `echo.storm.Storms`; design and numbers are in [echo-design.md](echo-design.md) §14.

- **Gate.** `Storms.gate(level, chunk)` answers `DISABLED` (`server.allowRecollectionStorms` false), `CAP` (`server.maxStormsPerDimension` storms already gathering or raging in that dimension; 0 blocks new ones), `CALM` (chunk not loaded or not at the fracture band), `MUTED` (a mute stone in the chunk), `WARDED` (scar glass or a Scar site within one chunk, `LoadedChunkMemory.stormProof`), `BUSY` (another storm within two chunks) or `OK`. It never loads a chunk.
- **Start.** `ImprintEvents.onPlayerTick` calls `Storms.onPlayerTick`: once every 20 ticks per player (staggered by entity id), a player standing in a fractured chunk rolls `spawnRates.stormAttemptChance` (0.02). The cost is one cached-band read per player per second. `ResidualShardItem` release on a fractured chunk calls `Storms.callByShard`, which starts a storm and adopts the freed residue.
- **Tick.** `ModGameEvents` calls `Storms.tick` on the server post tick. It iterates only the active storms (default cap 1 per dimension). A storm whose centre chunk is not loaded is skipped and freezes. The boss bar (a vanilla `ServerBossEvent`, players within 48 blocks) and the saved data update every 20 ticks. While gathering (200 ticks), a mute stone or ward in the centre chunk ends it as `CONTAINED`.
- **Wave.** Every 200 ticks, six times: `condenseArea` walks the imprint lists of the loaded, unmuted chunks in the 3×3 area once and turns the loudest graftable imprints into storm residues (at most 6 alive), then `Residues.stormWave` resolves each storm residue once (starved, fed to an echo, held while pinned, hushed, or acted out). That is at most 9 chunk lists and 6 entities every 10 seconds. A muted centre counts a wave twice.
- **End.** `SPENT` (nothing standing, nothing to condense), `PASSED` (fewer than 3 standing after the last wave; they become ordinary residues), `SCAR` (3 or more merge: `ScarSites.form` and one `ScarEntity`), `CONTAINED`, or `DISABLED`. Storm residues carry the storm id (`residue_storm`) and do not fester while their storm lives.
- **Scar site.** `ChunkMemory.scar` marks the centre chunk. With `mobGriefing`, `ScarSites.form` puts the `scar_heart` and up to 18 `scar_glass` only into air or replaceable blocks on sturdy ground. The residue pulse calls `ScarSites.reseed` for a player standing in a scar chunk: at most once per 24000 ticks, one strength-5 residue of a merged temper by the heart, skipped when muted or when a residue is already there. Breaking the heart calls `ScarSites.heal`.
